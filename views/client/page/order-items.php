  ...
  <div class="f-wrap">
    <div class="row">
      ...
      <ol class="ml-2 mt-0 breadcrumb bg-transparent breadcrumb-margin">
        ...
        <li class="breadcrumb-item active"><a href="<?= app_url('thong-tin-ca-nhan/don-hang/chi-tiet-don-hang?order_id=' . input_get('order_id')) ?> ">Chi tiết đơn hàng</a></li>
      </ol>
    </div>
    <div class="row my-5">
      <div class="col">
          ...
          <div class="card">
            <div class="card-header">
              ...
              <?php if ($order['order_status'] == 0) : ?>
                <a type="button" href="<?= app_url('thong-tin-ca-nhan/don-hang/them-chi-tiet?id=' . $order_id) ?>" class="btn btn-primary">Thêm sản phẩm vào đơn hàng</a><?php endif ?>
            </div>
            <div class="card-body">
                  ...
                  <?php foreach ($order_items as $item) : ?>
                    <tr>
                      <form action="<?= admin_url('don-hang/sua-chi-tiet') ?>" method="post">
                        ...
                        <td class="align-middle"><img width='100px' src="<?= asset('uploads/' . ($item['product_image'])) ?>" class="img-responsive" /></td>
                        <td class="align-middle"><?= priceVND($item['price']) ?></td><input type="hidden" value="<?= $item['price'] ?>">
                        <input type="hidden" name="id" value="<?= $item['id'] ?>">
                        <td class="align-middle"><input type="number" name="quantity" min="1" class="form-control w-100" value="<?= $item['quantity'] ?>" <?= ($order['order_status'] == 2 || $order['order_status'] == 3 || $order['order_status'] == 1) ? 'disabled' : '' ?> onchange="this.form.submit()"></td>
                        <td class="align-middle"><?= $item['color'] ?></td>
                        <td class="align-middle"><?= priceVND($item['total_price']) ?></td>
                        <?php if ($order['order_status'] == 0) : ?>
                          <td class="align-middle"><a class="btn btn-danger m-0 btn-sm " href="<?= admin_url('don-hang/xoa-chi-tiet?id=' . $item['id']) ?>" role="button"> Xóa</a></td>
                        <?php endif ?>
                      </form>
                    </tr>
                  <?php endforeach; ?>
